...
\begin{frame}{2. Dataset \& Feature Engineering}
    ...
    \small
    \begin{itemize}
        \item \textbf{Published Research Dataset (Kaggle):} Open-sourced on \href{https://www.kaggle.com/datasets/mouniksai/vehicular-misbehavior-dataset-edgetrust-vanet/data}{\textbf{\underline{Kaggle}}} (\texttt{EdgeTrust-VANET}) for community reproducibility and open-access V2X benchmarking.
        
        \item \textbf{Multi-Source ETL Pipeline (23{,}970 records):} Ingested, standardized, and harmonized 3 heterogeneous VANET simulation sources into a unified schema:
        \begin{itemize}
            \item \textbf{VeReMi (72.9\%)} + \textbf{EdgeTrust-VANET (20.9\%)} + \textbf{V-RADD (6.3\%)} [70.6\% Benign, 29.4\% Malicious].
        \end{itemize}
        ...
        \item \textbf{Audit \& Group Splitting:} Purged 4 post-hoc counters that cause live collapse (100\% $\rightarrow$ 86.05\% F1); enforced zero trajectory overlap via \textbf{GroupKFold by Vehicle ID} (70/15/15).
        
        \item \textbf{Result:} Genuine \textbf{97.74\% Weighted F1} on unseen vehicles; verified \textbf{90.99\% Live Accuracy} in OMNeT++/Veins RSU deployment.
    \end{itemize}
\end{frame}
